...
export default function FacturasPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.numero_factura || !formData.cliente_id) {
    ...
    try {
      if (editingId) {
        const { error: factError } = await supabase
          ...
          .update({
            ...formData,
            estado: formData.estado as "Borrador" | "Emitida" | "Pagada" | "Vencida" | "Cancelada",
            subtotal,
            igv,
            total,
          })
        ...
        await supabase.from("items_factura").delete().eq("factura_id", editingId);

        const { error: itemsError } = await supabase.from("items_factura").insert(
          items.map(item => ({
            factura_id: editingId,
  ...
  const handleEdit = async (factura: Factura) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Dialog open={isOpen} onOpenChange={(open) => { setIsOpen(open); if (!open) resetForm(); }}>
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingId ? "Editar" : "Nueva"} Factura</DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="bg-muted p-4 rounded-lg space-y-2">
                ...
                <div className="flex justify-between text-muted-foreground">
                  ...
                  <span>S/ {formData.monto_pagado.toFixed(2)}</span>
                </div>
                <div className="flex justify-between font-bold text-destructive">
                  <span>Saldo Pendiente:</span>
                  <span>S/ {(calculateTotals(items).total - formData.monto_pagado).toFixed(2)}</span>
                </div>
              </div>
              ...
              {facturas.length === 0 ? (
                ...
                facturas.map((factura) => (
                  <TableRow key={factura.id}>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button variant="ghost" size="sm" onClick={() => handleEdit(factura)}>
                          <Edit className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => setFacturaToDelete(factura)}>
      ...
      <AlertDialog open={!!facturaToDelete} onOpenChange={(open) => !open && setFacturaToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>¿Eliminar factura?</AlertDialogTitle>
            <AlertDialogDescription>
              Esta acción no se puede deshacer. Se eliminará permanentemente la factura 
              <span className="font-semibold"> {facturaToDelete?.serie}-{facturaToDelete?.numero_factura}</span>.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
